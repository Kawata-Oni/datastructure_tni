...
public class TreeHeight {
	...
	public static int heightDFS(Node node) {
		...
		while(!stack_node.isEmpty()) {
			...
			Integer current_depth = stack_depth.pop();
			maxHeight = Math.max(current_depth,maxHeight); // เอาอันที่สูงกว่าเก็บใน maxHeight
	...
	public static int heightBFS(Node node) {
		...
		if (node != null) {
			...
			while (!queue.isEmpty()) { // loop ใหญ่
				height++;
				int levelSize = queue.size();
				
				for (int i = 0; i<levelSize; i++) { // loop ย่อย
